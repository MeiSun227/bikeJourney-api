...
const addJourney = async (entry: Journey) => {
    const departure = new Date();
    const returnDate = new Date();
    const loanTime = new Date(returnDate.setMinutes(departure.getMinutes() + Math.random()));
    const duration = returnDate.getTime() - departure.getTime();
    entry.departure = departure;
    entry.return = loanTime;
    entry.duration = duration;

    const stationA = await stationData.findOneBy({ id: entry.departurestation_id}) as Station;
    const latA = stationA.y  * Math.PI/180;
    const lonA = stationA.x 
     const stationB = await stationData.findOneBy({ name: entry.returnstation_name }) as Station;
     const lonB = stationB.x 
     const latB = stationB.y  *Math.PI/180
     const R = 6371e3;
     const deltaLon = lonB - lonA;
     const deltaLambda = (deltaLon * Math.PI) / 180;
     const d = Math.acos(Math.sin(latA) * Math.sin(latB) + Math.cos(latA) * Math.cos(latB) * Math.cos(deltaLambda)) * R;
    entry.covereddistance = d
    const newJourneyEntry = await journeyData.create(entry);
